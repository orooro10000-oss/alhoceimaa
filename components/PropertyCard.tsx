...
import { Star, ChevronLeft, ChevronRight, Crown, Trophy, Gem, ShieldCheck, MapPin } from 'lucide-react';
    ...
    >
      <Link to={`/property/${property.id}`} className="block" draggable="false">
        ...
        <div className="transition-transform duration-300 active:scale-[0.98]">
            ...
            {/* Image Container */}
            <div 
              className="relative aspect-square overflow-hidden rounded-xl bg-gray-200 mb-3 shadow-sm group-hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-shadow duration-500 border border-gray-100"
            ...
            >
              ...
              {isGuestFavorite && (
                  <div className="absolute top-3 left-3 z-10 bg-white/95 backdrop-blur-sm px-2.5 py-1 rounded-full shadow-sm transform transition-transform duration-300 group-hover:scale-105">
                      <span className="text-[10px] font-bold text-gray-900">مفضّل للضيوف</span>
                  </div>
              )}

              {/* Special Badge (Crown, Trophy, etc.) */}
              {renderBadge()}
              ...
              {images.length > 1 && (
                <>
                  <button 
                    ...
                    className={`hidden md:flex absolute left-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-black p-1.5 rounded-full shadow-lg z-10 items-center justify-center border border-gray-100 transition-all duration-300 ${isHovered ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}
                  >
                    <ChevronLeft size={16} strokeWidth={2.5} />
                  </button>
                  <button 
                    onClick={handleNextClick}
                    className={`hidden md:flex absolute right-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-black p-1.5 rounded-full shadow-lg z-10 items-center justify-center border border-gray-100 transition-all duration-300 ${isHovered ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'}`}
                  >
